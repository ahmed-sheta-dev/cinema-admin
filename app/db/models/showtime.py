...
class Showtime(Base):
    ...
    movie_id: Mapped[int] = mapped_column(
        INTEGER(unsigned=True), ForeignKey("movies.id",ondelete="RESTRICT"), nullable=False, index=True
    )
    hall_id: Mapped[int] = mapped_column(
        INTEGER(unsigned=True), ForeignKey("halls.id",ondelete="RESTRICT"), nullable=False, index=True
    )
